...
def build_dashboard(inf_pages, clinic_pages):
    ...
    dm_pages = [p for p in inf_pages if get_prop(p, "DM Outreach") == True]
    email_pages = [p for p in inf_pages if get_prop(p, "DM Outreach") != True]

    total_inf = len(inf_pages)

    email_contacted = sum(1 for p in email_pages if get_prop(p, "Stage") not in [None, "Lead"])
    email_intake = sum(1 for p in email_pages if get_prop(p, "Stage") in [
        "Intake Survey Filled", "Product Delivered",
        "14-Day Survey Sent", "14-Day Survey Filled",
        "30-Day Survey Sent", "30-Day Survey Filled"
    ])
    email_delivered = sum(1 for p in email_pages if get_prop(p, "Stage") in [
        "Product Delivered", "14-Day Survey Sent", "14-Day Survey Filled",
        "30-Day Survey Sent", "30-Day Survey Filled"
    ])
    email_survey_14 = sum(1 for p in email_pages if get_prop(p, "Stage") in [
        "14-Day Survey Sent", "14-Day Survey Filled"
    ])
    email_survey_30 = sum(1 for p in email_pages if get_prop(p, "Stage") in [
        "30-Day Survey Sent", "30-Day Survey Filled"
    ])
    email_declined = sum(1 for p in email_pages if get_prop(p, "Stage") == "Declined")

    dm_sent = len(dm_pages)
    dm_responded = sum(1 for p in dm_pages if get_prop(p, "DM Response") == True)
    dm_interested = sum(1 for p in dm_pages if get_prop(p, "DM Decision") == "Interested")
    dm_converted = sum(1 for p in dm_pages if get_prop(p, "Stage") in [
        "Intake Survey Filled", "Product Delivered",
        "14-Day Survey Sent", "14-Day Survey Filled",
        "30-Day Survey Sent", "30-Day Survey Filled"
    ])
    dm_not_now = sum(1 for p in dm_pages if get_prop(p, "DM Decision") == "Not Now")
    dm_declined = sum(1 for p in dm_pages if get_prop(p, "DM Decision") == "Declined")
    dm_no_resp = sum(1 for p in dm_pages if get_prop(p, "DM Decision") == "No Response" or get_prop(p, "DM Decision") is None)
    ...
    seeded = [p for p in inf_pages if get_prop(p, "Stage") in [
        "Product Delivered", "14-Day Survey Sent", "14-Day Survey Filled",
        "30-Day Survey Sent", "30-Day Survey Filled"
    ]]
    seeded_rows = ""
    ...
    <div class="seed-row">
      ...
      <div class="seed-stage"><span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:{sb};color:{sc}">{stage}</span></div>
    </div>'''

    def dm_badge(p):
        decision = get_prop(p, "DM Decision")
        stage = get_prop(p, "Stage") or ""
        if stage in ["Intake Survey Filled","Product Delivered","14-Day Survey Sent","14-Day Survey Filled","30-Day Survey Sent","30-Day Survey Filled"]:
            return '<span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:rgba(192,38,168,.15);color:var(--pink)">✅ Converted</span>', "var(--pink)"
        if decision == "Interested":
            return '<span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:rgba(99,153,34,.15);color:var(--green)">🟢 Interested</span>', "var(--green)"
        if decision == "Not Now":
            return '<span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:rgba(186,117,23,.15);color:var(--amber)">🟡 Not Now</span>', "var(--amber)"
        if decision == "Declined":
            return '<span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:rgba(226,75,74,.15);color:var(--red)">🔴 Declined</span>', "var(--red)"
        return '<span style="font-size:9px;font-weight:700;padding:2px 8px;border-radius:3px;background:rgba(136,135,128,.15);color:var(--grey)">⬜ No Response</span>', "var(--border)"

    dm_rows_html = ""
    for p in dm_pages:
        name = get_prop(p, "Name") or "Unknown"
        handle = get_prop(p, "Handle") or ""
        note = get_prop(p, "Note") or ""
    ...
    <div class="dm-row" style="border-left:3px solid {border}">
      ...
      <div class="dm-note">{note[:90]}</div>
    </div>'''

    def bar(label, count, total, color, dim=False):
        pct = max(4, round(count / max(total, 1) * 100))
        op = "opacity:0.4;" if dim and count == 0 else ""
        return f'''<div class="f-row">
      <div class="f-lbl">{label}</div>
      <div class="f-track"><div class="f-bar" style="width:{pct}%;background:{color};{op}min-width:28px">{count}</div></div>
...
<div class="page active" id="inf">
  <div class="alert">Auto-refreshed daily at 11:00 Prague time · Last update: {today_short}</div>
  <div class="ch-header">
    <div class="ch-title">📧 Email Outreach</div>
    <div class="ch-pill pill-e">{email_contacted} contacted · {total_inf - len(dm_pages)} total</div>
  </div>
  <div class="stat-row g4">
    <div class="stat"><div class="stat-label">Total in DB</div><div class="stat-value" style="color:var(--purple)">{total_inf - len(dm_pages)}</div><div class="stat-sub">{email_contacted} contacted</div></div>
    <div class="stat"><div class="stat-label">Intake Filled</div><div class="stat-value" style="color:var(--amber)">{email_intake}</div></div>
    <div class="stat"><div class="stat-label">Product Delivered</div><div class="stat-value" style="color:var(--green)">{email_delivered}</div></div>
    <div class="stat"><div class="stat-label">Declined</div><div class="stat-value" style="color:var(--red)">{email_declined}</div></div>
  </div>
  <div class="slbl">Email Funnel</div>
  <div class="funnel">{email_funnel}</div>
  <div class="slbl">Seeded Influencers ({len(seeded)})</div>
  {'<div class="empty">No influencers seeded yet.</div>' if not seeded_rows else f\'\'\'
  ...
  <div class="seed-list">{seeded_rows}</div>\'\'\'}
  <div class="div"></div>
  <div class="ch-header">
    <div class="ch-title">📸 Instagram DM</div>
    <div class="ch-pill pill-d">{dm_sent} DMs · {resp_rate}% responded</div>
  </div>
  <div class="stat-row g4">
    <div class="stat"><div class="stat-label">DMs Sent</div><div class="stat-value" style="color:var(--pink)">{dm_sent}</div></div>
    <div class="stat"><div class="stat-label">Responded</div><div class="stat-value" style="color:var(--teal)">{dm_responded}</div><div class="stat-sub">{resp_rate}% response rate</div></div>
    <div class="stat"><div class="stat-label">Interested</div><div class="stat-value" style="color:var(--green)">{dm_interested}</div><div class="stat-sub">{dm_converted} converted</div></div>
    <div class="stat"><div class="stat-label">No Response</div><div class="stat-value" style="color:var(--grey)">{dm_no_resp}</div></div>
  </div>
  <div class="slbl">DM Tracker</div>
  <div class="dm-list">{'<div class="empty">No DMs recorded yet.</div>' if not dm_rows_html else dm_rows_html}</div>
  <div class="footer">Last updated: {today_short} · {DASHBOARD_URL}</div>
</div>

<div class="page" id="cli">
  <div class="ch-header">
    <div class="ch-title">🏥 UK Clinics</div>
    <div class="ch-pill pill-c">{total_clinics} contacted · 2 batches</div>
  </div>
  <div class="stat-row g4">
    <div class="stat"><div class="stat-label">Total in DB</div><div class="stat-value" style="color:var(--teal)">{total_clinics}</div></div>
    <div class="stat"><div class="stat-label">Outreach Sent</div><div class="stat-value" style="color:var(--teal)">{total_clinics}</div></div>
    <div class="stat"><div class="stat-label">Meeting Booked</div><div class="stat-value" style="color:var(--amber)">{clinic_meeting}</div></div>
    <div class="stat"><div class="stat-label">Declined</div><div class="stat-value" style="color:var(--red)">{clinic_declined}</div></div>
  </div>
  <div class="slbl">Clinic Funnel</div>
  <div class="funnel">{clinic_funnel}</div>
  <div class="stat-row g2">
    <div class="stat" style="border-left:3px solid var(--teal)"><div class="stat-label">Batch 1 — 21–22 Jun</div><div class="stat-value" style="color:var(--teal)">25</div><div class="stat-sub">Standard subject line · FU1 due 30 Jun</div></div>
    <div class="stat" style="border-left:3px solid var(--dark-purple)"><div class="stat-label">Batch 2 — 25 Jun · A/B Test</div><div class="stat-value" style="color:var(--dark-purple)">20</div><div class="stat-sub">Subject: "Strange ask" · FU1 due 7 Jul</div></div>
  </div>
  <div class="footer">Last updated: {today_short} · {DASHBOARD_URL}</div>
</div>
...
if __name__ == "__main__":
    ...
    print(f"  Influencers: {len(inf_pages)}, Clinics: {len(clinic_pages)}")
